...
/// ============================================================================
/// YUV CONVERTER - ✅ OPTIMISÉ pour Android
/// ============================================================================
class YUVConverter {
  static Uint8List convertYUV420ToNV21(CameraImage image) {
    ...
    // ✅ Copier le plan Y avec stride correct
...
/// ============================================================================
/// 🆕 HYBRID FACE VALIDATOR - Fonctionne sur Android ET iOS
/// ============================================================================
///
/// Principe : ML Kit retourne les boundingBox dans l'espace de l'image BRUTE
/// (non-rotée). On doit prendre en compte la rotation pour valider correctement.
///
/// Sur Android : L'image peut être 1920x1080 (landscape) même en mode portrait
/// Sur iOS : L'orientation est gérée automatiquement par le framework
///
class FaceValidator {
  ...
  FaceValidationResult validate(Face face) {
    ...
    // ✅ ÉTAPE 1 : Normaliser les coordonnées selon la rotation
    final normalized = _normalizeCoordinates(rect);

    // ✅ ÉTAPE 2 : Calculer les dimensions effectives après rotation
    ...
    // ✅ ÉTAPE 3 : Calculer le centre et les offsets
    final double faceCenterX = normalized.centerX;
    final double faceCenterY = normalized.centerY;
    final double faceHeight = normalized.height;

    final double imageCenterX = effectiveWidth / 2;
    final double imageCenterY = effectiveHeight / 2;

    // ✅ ÉTAPE 4 : Offsets normalisés (0 à 1)
    final double offsetX = (faceCenterX - imageCenterX).abs() / effectiveWidth;
    final double offsetY = (faceCenterY - imageCenterY).abs() / effectiveHeight;
    final double faceRatio = faceHeight / effectiveHeight;

    // ✅ ÉTAPE 5 : Validation anti-NaN
    if (offsetX.isNaN ||
        offsetY.isNaN ||
        faceRatio.isNaN ||
        offsetX > 1.0 ||
    ...
    debugStr += " rot:${_rotationToDegrees()}°";
    debugStr += " eff:${effectiveWidth}x${effectiveHeight}]";
  ...
  /// 🆕 Normalise les coordonnées du boundingBox selon la rotation
  ///
  /// Sur Android, ML Kit retourne les coordonnées dans l'espace de l'image
  /// brute (non-rotée). On doit appliquer la rotation pour avoir les
  /// coordonnées dans l'espace de l'image telle qu'elle sera affichée.
  ///
  /// Sur iOS, les coordonnées sont déjà correctes grâce au framework.
  ///
  _NormalizedRect _normalizeCoordinates(Rect rect) {
    // ✅ Sur iOS, pas besoin de transformation
    ...
    // ✅ Sur Android, appliquer la rotation
    // ML Kit retourne les boundingBox dans l'espace [imageWidth x imageHeight]
    // On doit transformer selon la rotation
    switch (rotation) {
      case InputImageRotation.rotation90deg:
        // Portrait normal sur Android (capteur landscape)
        // Transformation : (x, y) → (y, imageWidth - x)
        return _NormalizedRect(
      ...
      case InputImageRotation.rotation270deg:
        // Portrait inversé
        // Transformation : (x, y) → (imageHeight - y, x)
        return _NormalizedRect(
          centerX: imageHeight - (rect.top + rect.height / 2),
          centerY: rect.left + rect.width / 2,
          width: rect.height,
          height: rect.width,
        );
      ...
      case InputImageRotation.rotation180deg:
        // Landscape inversé
        // Transformation : (x, y) → (imageWidth - x, imageHeight - y)
        return _NormalizedRect(
          centerX: imageWidth - (rect.left + rect.width / 2),
          centerY: imageHeight - (rect.top + rect.height / 2),
          width: rect.width,
          height: rect.height,
        );
      ...
      default:
        // Landscape normal - pas de transformation
        return _NormalizedRect(
          centerX: rect.left + rect.width / 2,
          centerY: rect.top + rect.height / 2,
          width: rect.width,
...
/// Coordonnées normalisées après transformation de rotation
...
class _CameraWidgetState extends State<CameraWidget>
  ...
  void _updateFeedback(List<Face> faces) {
    ...
    if (_lastFeedbackUpdate != null &&
        now.difference(_lastFeedbackUpdate!) <
            FaceDetectionConfig.feedbackThrottle) {
      return;
    }
    ...
    setState(() {
      ...
      // ✅ NOUVEAU : Validator hybride avec rotation
      final validator = FaceValidator(
        imageWidth: rawW,
        imageHeight: rawH,
        rotation: rot,
        isFrontCamera: !_state.isRearCamera,
      );
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Semantics(
          ...
          label: canCapture
              ? 'Prendre une photo'
              : 'Positionnez votre visage pour activer',
          enabled: canCapture,
