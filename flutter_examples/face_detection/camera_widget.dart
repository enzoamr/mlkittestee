...
class YUVConverter {
  static Uint8List convertYUV420ToNV21(CameraImage image) {
    ...
    // ✅ FIX: Copier correctement les plans UV en format NV21 (VUVUVU...)
...
class CameraState {
  ...
  bool get shouldProcessFrames =>
      lifecycle == CameraLifecycleState.ready && isStreamingImages;
}
...
class _CameraWidgetState extends State<CameraWidget>
  ...
  Future<void> _setupCameraController(CameraDescription camera) async {
    ...
    // ✅ FIX: Platform check avec kIsWeb en premier
  ...
  void _startFaceDetection() {
    if (!_state.canStartStream || _cameraController == null) {
      CameraLogger.log(
          "⚠️ Cannot start stream: lifecycle=${_state.lifecycle}, canStart=${_state.canStartStream}");
      return;
    }
      ...
      _cameraController!.startImageStream((CameraImage image) async {
        ...
        try {
          if (_faceDetector != null) {
            ...
            // ✅ FIX: Vérification mounted avant setState
  ...
  Future<List<Face>> _detectFacesFromCameraImage(CameraImage image) async {
    try {
      ...
      // ✅ FIX: kIsWeb check en premier
      if (kIsWeb) {
        // Web ne supporte pas le streaming, retour vide
        return [];
      } else if (Platform.isAndroid) {
  ...
  // ✅ FIX: Stopper proprement le stream avec vérification mounted
  ...
  Future<void> _switchCamera() async {
    ...
    try {
      ...
      // ✅ FIX: Vérifier mounted après await
      if (!mounted) return;
  ...
  // ✅ FIX: Gérer proprement la prise de photo avec vérifications mounted
  Future<void> _takePhoto() async {
    ...
    try {
      ...
      // ✅ FIX: Vérifier mounted après chaque opération async
      if (!mounted) return;
  ...
  Future<void> _openGallery() async {
    ...
    try {
      ...
      CameraLogger.log(
          "✅ Navigation galerie déclenchée, widget va être détruit");
    } catch (e) {
  ...
  // ✅ FIX: Vérifications mounted après chaque await
  Future<void> _resumeCamera() async {
    ...
    if (_state.lifecycle == CameraLifecycleState.paused && mounted) {
      ...
      // ✅ Revérifier mounted après le delay
      if (mounted && _state.lifecycle == CameraLifecycleState.ready) {
        _startFaceDetection();
      }
    }
  ...
  // ✅ FIX CRITIQUE: Dispose propre et SYNCHRONE
  @override
  void dispose() {
    ...
    // ✅ Cleanup SYNCHRONE - pas d'async dans dispose!
    if (_state.isStreamingImages && _cameraController != null) {
      try {
  ...
  // ✅ NOUVEAU: Bouton avec Semantics pour accessibilité
