...
/// ============================================================================
/// COORDINATE TRANSFORMER - Pour Android uniquement
/// ============================================================================
class CoordinateTransformer {
  ...
  void _calculateTransformation() {
    ...
    CameraLogger.log(
        "🔧 Transformer: ${imageWidth}x${imageHeight} → ${viewSize.width.toStringAsFixed(1)}x${viewSize.height.toStringAsFixed(1)}, scale=$scaleFactor");
  }
...
/// ============================================================================
/// FACE VALIDATOR - Hybride iOS/Android
/// ============================================================================
class FaceValidator {
  final int imageWidth;
  final int imageHeight;
  ...
  FaceValidator({
    ...
    // ✅ Créer le transformer UNIQUEMENT pour Android
    if (!kIsWeb && Platform.isAndroid && viewSize != null) {
      _transformer = CoordinateTransformer(
        imageWidth: imageWidth,
        imageHeight: imageHeight,
        viewSize: viewSize!,
        isImageFlipped: isFrontCamera,
      );
    }
  }
  ...
  FaceValidationResult validate(Face face) {
    ...
    // ✅ DEUX MÉTHODES DE CALCUL SELON LA PLATEFORME
    ...
    if (!kIsWeb && Platform.isAndroid && _transformer != null) {
      // ✅ ANDROID : Calcul dans l'espace ÉCRAN avec CoordinateTransformer
      faceCenterX = _transformer!.translateX(rect.left + rect.width / 2);
      faceCenterY = _transformer!.translateY(rect.top + rect.height / 2);
      faceHeight = _transformer!.scale(rect.height.toDouble());

      centerX = viewSize!.width / 2;
      centerY = viewSize!.height / 2;
      refWidth = viewSize!.width;
      refHeight = viewSize!.height;

      CameraLogger.log("📱 Android - Calcul ÉCRAN avec transformer");
    } else {
      // ✅ iOS : Calcul dans l'espace IMAGE (simple et direct)
      ...
      refWidth = imageWidth.toDouble();
      refHeight = imageHeight.toDouble();

      CameraLogger.log("🍎 iOS - Calcul IMAGE direct");
    }

    // ✅ Validation anti-NaN
    if (faceCenterX.isNaN ||
        faceCenterY.isNaN ||
        faceHeight.isNaN ||
        faceCenterX > refWidth * 2 ||
        faceCenterY > refHeight * 2) {
      CameraLogger.error(
          "⚠️ Valeurs invalides: centerX=$faceCenterX, centerY=$faceCenterY, height=$faceHeight");
      return FaceValidationResult.noFace();
    }

    // ✅ Calcul des offsets normalisés
    final double offsetX = (faceCenterX - centerX).abs() / refWidth;
    final double offsetY = (faceCenterY - centerY).abs() / refHeight;
    final double faceRatio = faceHeight / refHeight;
    ...
    if (offsetX.isNaN ||
        ...
        faceRatio < 0.01) {
      CameraLogger.error(
          "⚠️ Ratios invalides: offsetX=$offsetX, offsetY=$offsetY, faceRatio=$faceRatio");
      return FaceValidationResult.noFace();
    }
    ...
    debugStr +=
        "[${imageWidth}x${imageHeight} → ${refWidth.toInt()}x${refHeight.toInt()}]";
...
class _CameraWidgetState extends State<CameraWidget>
  ...
  void _updateFeedback(List<Face> faces) {
    ...
    setState(() {
      ...
      // ✅ Créer validator avec viewSize pour Android, sans pour iOS
      final viewSize = (_previewWidth != null && _previewHeight != null)
          ? Size(_previewWidth!, _previewHeight!)
          : null;
