...
/// ============================================================================
/// CONFIGURATION SIMPLIFIÉE - Plus besoin de calibrations complexes!
/// ============================================================================
...
/// ============================================================================
/// HELPERS - Utilitaires pour la compression et le crop
/// ============================================================================
Future<Uint8List> compressImage(String filePath) async {
  if (kIsWeb) {
    throw UnimplementedError("La compression d'image n'est pas implémentée sur le web.");
  }
...
/// ============================================================================
/// TRANSFORMATION COORDINATOR - Exactement comme GraphicOverlay de Google!
/// ============================================================================
class CoordinateTransformer {
  ...
  /// Calcul de transformation - EXACTEMENT comme GraphicOverlay.java ligne 271-297
  void _calculateTransformation() {
    ...
    if (viewAspectRatio > imageAspectRatio) {
      ...
      postScaleHeightOffset = (viewSize.width / imageAspectRatio - viewSize.height) / 2;
      postScaleWidthOffset = 0;
    } else {
      // L'image doit être croppée horizontalement
      scaleFactor = viewSize.height / imageHeight;
      postScaleWidthOffset = (viewSize.height * imageAspectRatio - viewSize.width) / 2;
      postScaleHeightOffset = 0;
    }

    debugPrint("🔧 TRANSFORMER:");
    debugPrint("  - Image: ${imageWidth}x${imageHeight}");
    debugPrint("  - View: ${viewSize.width.toStringAsFixed(1)}x${viewSize.height.toStringAsFixed(1)}");
    debugPrint("  - Scale factor: ${scaleFactor.toStringAsFixed(4)}");
    debugPrint("  - Width offset: ${postScaleWidthOffset.toStringAsFixed(1)}");
    debugPrint("  - Height offset: ${postScaleHeightOffset.toStringAsFixed(1)}");
    debugPrint("  - Flipped: $isImageFlipped");
  }

  /// Scale - EXACTEMENT comme Graphic.scale() ligne 109-111
  double scale(double imagePixel) {
    return imagePixel * scaleFactor;
  }

  /// TranslateX - EXACTEMENT comme Graphic.translateX() ligne 125-131
  ...
  /// TranslateY - EXACTEMENT comme Graphic.translateY() ligne 136-138
...
/// ============================================================================
/// RÉSULTAT DE VALIDATION - Structure pour retourner les résultats
/// ============================================================================
...
/// ============================================================================
/// VALIDATEUR DE VISAGE - Logique de validation simplifiée et robuste
/// ============================================================================
class FaceValidator {
  ...
  FaceValidationResult validate(Face face) {
    ...
    // Étape 1: Transformer les coordonnées - EXACTEMENT comme FaceGraphic.kt ligne 65-73
    final double faceCenterX = transformer.translateX(rect.left + rect.width / 2);
    final double faceCenterY = transformer.translateY(rect.top + rect.height / 2);
...
class _CameraWidgetState extends State<CameraWidget>
  ...
  bool _isRearCamera = false; // Par défaut caméra frontale!
  ...
  void _startFaceDetection() {
    ...
    try {
      _cameraController!.startImageStream((CameraImage image) async {
        ...
        // Traiter seulement 1 frame sur 3 pour la performance
    ...
    setState(() {
      ...
      if (_transformer == null && _cameraController != null) {
        ...
        _transformer = CoordinateTransformer(
          ...
          viewSize: screenSize,
          isImageFlipped: !_isRearCamera, // Flip pour caméra frontale
        );
      }
...
class MinimalGuidePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawCorner(canvas, cornerPaint, ovalRect.left, ovalRect.top, cornerLength, cornerOffset, isTopLeft: true);
    _drawCorner(canvas, cornerPaint, ovalRect.right, ovalRect.top, cornerLength, cornerOffset, isTopRight: true);
    _drawCorner(canvas, cornerPaint, ovalRect.left, ovalRect.bottom, cornerLength, cornerOffset, isBottomLeft: true);
    _drawCorner(canvas, cornerPaint, ovalRect.right, ovalRect.bottom, cornerLength, cornerOffset, isBottomRight: true);
